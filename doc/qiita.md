...
# はじめに
前の年の会津アドベントカレンダーではサウンドプログラミングの先駆けでwavファイルのBPMを解析する記事を[書きました](https://qiita.com/stringamp/items/35ea7cca7a70f99f8de3)が、今年は本格的にサウンドプログラミングに取り組んでみます。
...
## Plate
ここから人工的なリバーブの生成法、その中でもPlate Reverbは1950年代に誕生。名前から直感的に明らかな様に、鉄板の揺れを利用してリバーブを実現する手法。
...
（今回採用した畳み込み手法が上の手法とは異なる物だったので、結局上のコードはどこにも使いませんでした）
...
以上の点を踏まえて、この記事の目的である畳み込み演算を利用した残業効果の実装は大きく分けて以下のステップで実装出来そうです。
...
- STEP 3. 畳み込み
  ...
  - STEP 3.3. 周波数領域でインパルス応答と入力波形を畳み込み
...
が、今回は時間が無かったのでFFTに関してはソースコードを拾ってきました。
リンク：
...
このステップは一気に書いてしまいます。
入力波形に関しては、「拡張入力波形から2フレーム抜き出して畳み込む」という操作を先頭から末尾まで行う必要があります。
つまり、フレーム数を$k$と置くと、畳み込む回数は全体で$k-1$回必要なので、以下の様に書けるでしょう。
...
おそらくこの記事を上げた後にも記事ソースコード共にアップデートを繰り返して行くと思いますが、今回は取り敢えず動くものが出来た、という時点で上げさせて貰いました。
